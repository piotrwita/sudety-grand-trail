...
import { MapIcon, FacebookIcon } from '@/components/icons'
			...
			<div className='container-custom'>
				<div className='flex items-center justify-between h-16'>
					...
					<Link href='/' className='flex items-center space-x-3'>
						...
						<span className='font-display font-bold text-lg text-cream hidden sm:block'>{siteConfig.name}</span>
					</Link>
						...
						{siteConfig.navigation.map(item => (
							<Link
								...
								className={`relative font-medium transition-colors duration-200 ${
									pathname === item.href ? 'text-accent' : 'text-cream/90 hover:text-cream'
								}`}>
								<div className='flex items-center space-x-2'>
									{'isLive' in item && item.isLive && <div className='w-2 h-2 bg-accent rounded-full animate-pulse' />}
