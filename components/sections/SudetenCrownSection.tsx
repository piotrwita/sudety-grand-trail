...
const SudetenCrownSection = () => {
	...
	const togglePeak = (rangeId: number) => {
		setSelectedPeaks(prev => (prev.includes(rangeId) ? prev.filter(id => id !== rangeId) : [...prev, rangeId]))
	}
					...
					<div className='grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
						{sudetenRanges.map((range, index) => (
							<motion.div
								...
								{/* Rank Badge */}
								<div
									className={`absolute top-4 right-4 w-8 h-8 bg-gradient-to-br ${getRankColor(
										range.rank
									)} text-cream rounded-full flex items-center justify-center font-display font-bold text-sm shadow-vintage`}>
									{range.rank}
								</div>
								...
								{/* Country Flag */}
								<div className='absolute top-4 left-4 text-2xl'>{getCountryFlag(range.country)}</div>

								{/* Peak Icon */}
								<div className='text-center mb-4 mt-8'>
									<div className='text-4xl mb-2'>🏔️</div>
									<div className='stats-number text-2xl text-forest-700'>{range.elevation}m</div>
								</div>

								{/* Range Name */}
								<h3 className='section-title text-lg mb-2 text-center leading-tight'>{range.name}</h3>

								{/* Peak Name */}
								<p className='text-center text-mountain-600 font-bold mb-4'>{range.peak}</p>
					...
					<div className='space-y-8'>
						...
						<div className='text-center'>
							...
							<div className='flex flex-wrap justify-center gap-4 mb-8'>
								...
								<button onClick={() => setSelectedPeaks(sudetenRanges.map(r => r.id))} className='btn-secondary'>
									Pełna Korona (24)
								</button>
								<button
						...
						<div className='grid lg:grid-cols-3 gap-8'>
							...
							<div className='lg:col-span-2'>
								<h3 className='section-title text-2xl mb-6 text-left'>Wybierz Szczyty do Zdobycia</h3>
														...
														<div className='flex items-center space-x-2'>
															...
															<div
																className={`w-6 h-6 bg-gradient-to-br ${getRankColor(
																	range.rank
																)} text-cream rounded-full flex items-center justify-center text-xs font-bold`}>
																{range.rank}
															</div>
														</div>
													</div>
							...
							<div className='space-y-6'>
								<h3 className='section-title text-2xl mb-6 text-left'>Twój Plan Wędrówki</h3>
								...
								{selectedPeaks.length > 0 ? (
									<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className='space-y-6'>
										...
										<div className='grid grid-cols-2 gap-4'>
											...
											<div className='card-vintage p-4 text-center'>
												<div
													className={`stats-number text-2xl mb-1 ${
														calculateTripStats().difficulty === 'Ekstremalny'
															? 'text-accent'
															: calculateTripStats().difficulty === 'Bardzo trudny'
															? 'text-earth-700'
											...
											<motion.div
												...
												<div className='text-center'>
													...
													<p className='text-sm text-mountain-700 mb-2'>
														Zdobędziesz{' '}
														<span className='font-bold text-accent'>{calculateTripStats().kgpCount} z 16</span>{' '}
														sudeckich szczytów Korony Gór Polski!
													</p>
													<div className='text-xs text-mountain-600'>
														{calculateTripStats().kgpPeaks.map((peak, index) => (
															<span key={peak.id}>
																{peak.name} ({peak.code}){index < calculateTripStats().kgpPeaks.length - 1 ? ', ' : ''}
															</span>
														))}
													</div>
												</div>
										...
										<div className='card-vintage p-6'>
											...
											<div className='bg-forest-100 rounded-full h-4 mb-2 relative overflow-hidden'>
												<div
													...
													style={{
														width: `${
															((calculateTripStats().maxElevation - calculateTripStats().minElevation) / (1603 - 714)) *
															100
														}%`,
													}}
												/>
										...
										<div className='space-y-3'>
											<button className='btn-primary w-full'>Pobierz Plan Wędrówki</button>
											<button className='btn-secondary w-full'>Udostępnij Plan</button>
										</div>
									</motion.div>
								) : (
